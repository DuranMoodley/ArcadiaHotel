...
//http://duran.dx.am/SaveEmployee.php?name=John&surname=Doe&contactNumber=3333334&emailAddress=duronsfo&duty=Cleaner&status=Leave&shift=8:00-14:00
 fetch('http://duran.dx.am/SaveEmployee.php?name=' +Name+'&surname=' + Surname + '&duty=' + Duty +'&status='+ Status + '&shift=' + Shift + '&emailAddress=' + EmailAddress + '&contactNumber=' + ContactNumber, fetchConfig)
 //.then(res)
 .then(e => {
 ...
 // await AsyncStorage.setItem('WeighbridgeName', item.WeighbridgeName);
 // await AsyncStorage.setItem('WeighbridgeId', JSON.stringify(item.WeighbridgeId));
 
 console.log(item.WeighbridgeId);
 setGlobal({
 ...global,
 HaulierId : item.HaulierId
 })
 
 props.navigation.navigate('VehicleDriverSelection')
 }
